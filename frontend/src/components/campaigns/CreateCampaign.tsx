...
// FIX: Removed isOpen from props, as the parent now controls this.
interface CreateCampaignProps {
  onClose: () => void;
  onSuccess?: () => void;
  onUseAIMessages: (campaignType: string, targetSegment: any) => void; // Assuming this prop is still needed
}

const CreateCampaign: React.FC<CreateCampaignProps> = ({
  onClose,
  onSuccess,
  onUseAIMessages // Assuming this prop is still needed
}) => {
  ...
  const handleSubmit = () => {
    createMutation.mutate(formData, {
      onSuccess: () => {
        onClose(); // Use the passed-in onClose prop
        setStep(1);
        setFormData({
          segmentId: 0,
          name: '',
  ...
  // FIX: handleClose now directly calls the prop from the parent.
  const handleClose = () => {
    onClose();
    setStep(1);
    setFormData({
  ...
  // FIX: Removed the <Modal> wrapper. The component now returns the form content directly.
  return (
    <div className="space-y-6"> {/* Replaced p-6 with space-y-6 for consistency */}
      {/* Progress Steps */}
          ...
          <div>
            ...
            {formData.useAI ? (
              <div className="space-y-4">
                ...
                <MessageGenerator
                  campaignType={formData.campaignObjective}
                  audience={selectedSegment}
                  onMessageGenerated={(msg) => setFormData(prev => ({ ...prev, message: msg.message }))}
                />
              </div>
            ) : (
              <div>
