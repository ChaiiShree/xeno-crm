  ...
  try {
    ...
    if (createMethod === 'ai') {
      if (!formData.nlpQuery.trim()) {
        toast.error('AI query is required');
        return;
      }
      // ONLY send the nlpQuery. The backend will generate the rules.
      payload.nlpQuery = formData.nlpQuery.trim();

    } else { // Manual method
      if (!isValidRules() || !rules) {
        toast.error('Please define valid segment rules');
        return;
      }
      payload.rules = rules;
    }
...
const handleSegmentGenerated = (generatedData: any) => {
    ...
    // The `setRules` call is no longer needed for segment creation.
    // It is only used for the AudiencePreview component.
